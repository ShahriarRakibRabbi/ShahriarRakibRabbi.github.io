 ...
 * Handles editing functionality for portfolio content
 * @version: 2.0
 * @author: SRR
 */
...
const EditorModule = (function () {
  ...
  function loadGitHubCredentials() {
    ...
    if (credentials) {
      ...
      if (domElements.githubTokenInput)
        domElements.githubTokenInput.value = gitHubToken;
      if (domElements.githubRepoInput)
        domElements.githubRepoInput.value = repoInfo.repo;
      if (domElements.githubUsernameInput)
        domElements.githubUsernameInput.value = repoInfo.username;
    }
  }
  ...
  function saveGitHubCredentials() {
    if (
      domElements.githubTokenInput &&
      domElements.githubRepoInput &&
      domElements.githubUsernameInput
    ) {
  ...
  function processUploads() {
    ...
    const progressContainer = currentForm.querySelector(
    ...
    const uploadPromises = uploadQueue.map((upload, index) => {
      return new Promise((resolve, reject) => {
        // Update progress bar
        const progressBar = currentForm.querySelector(
          `.upload-progress[data-index="${index}"]`
        );
        if (progressBar) {
        ...
        compressImageIfNeeded(upload.file)
          ...
          .then((base64Content) => {
            ...
            // Upload to GitHub
            const fileName = `assets/images/${
              upload.targetPath
            }/${Date.now()}-${upload.file.name}`;
  ...
  function addNewItem(container) {
    ...
    const itemElements = clone.querySelectorAll(
      "[id], [for], [data-target], [name]"
    );
    ...
    itemElements.forEach((element) => {
      ...
      if (element.getAttribute("for")) {
        element.setAttribute(
          "for",
          element.getAttribute("for").replace("__ID__", itemId)
        );
      }
      ...
      if (element.getAttribute("data-target")) {
        element.setAttribute(
          "data-target",
          element.getAttribute("data-target").replace("__ID__", itemId)
        );
      }
  ...
  function extractProjectsData() {
    ...
    const projects = Array.from(projectItems).map((item, index) => {
      ...
      const categoriesInput = item.querySelector(
        ".project-categories-input"
      ).value;
      ...
      const technologiesInput = item.querySelector(
    ...
    const skills = Array.from(categoryItems).map((item) => {
      ...
      const items = Array.from(skillItems).map((skillItem) => {
        ...
        const level = parseInt(
          skillItem.querySelector(".skill-level-input").value,
          10
        );
        const levelText = skillItem.querySelector(
          ".skill-level-text-input"
        ).value;
        const icon = skillItem.querySelector(".skill-icon-input").value;
        const description = skillItem.querySelector(
          ".skill-description-input"
        ).value;
        const yearsExperience = parseInt(
          skillItem.querySelector(".skill-years-input").value,
          10
        );
  ...
  function extractEducationData() {
    ...
    return Array.from(educationItems).map((item) => {
      return {
        ...
        achievements: item
          .querySelector(".education-achievements-input")
          .value.split("\n")
          .map((line) => line.trim())
          .filter((line) => line),
      };
    ...
    return Array.from(experienceItems).map((item) => {
      return {
        ...
        responsibilities: item
          .querySelector(".experience-responsibilities-input")
          .value.split("\n")
          .map((line) => line.trim())
          .filter((line) => line),
      };
  ...
  function extractCertificatesData() {
    ...
    return Array.from(certificateItems).map((item) => {
      return {
        ...
        skills: item
          .querySelector(".certificate-skills-input")
          .value.split(",")
  ...
  function extractTestimonialsData() {
    ...
    return Array.from(testimonialItems).map((item, index) => {
      ...
      return {
        ...
        rating: parseInt(
          item.querySelector(".testimonial-rating-input").value,
          10
        ),
  ...
  function populateProjectsEditor(projects) {
    ...
    projects.forEach((project) => {
      ...
      clone.querySelector(".project-description-input").value =
        project.description || "";
      clone.querySelector(".project-thumbnail-input").value =
        project.thumbnailUrl || "";

      // Set image URLs as comma-separated string
      if (project.imageUrls && Array.isArray(project.imageUrls)) {
        clone.querySelector(".project-images-input").value =
          project.imageUrls.join(", ");
      }

      // Set categories as comma-separated string
      if (project.categories && Array.isArray(project.categories)) {
        clone.querySelector(".project-categories-input").value =
          project.categories.join(", ");
      }

      // Set technologies as comma-separated string
      if (project.technologies && Array.isArray(project.technologies)) {
        clone.querySelector(".project-technologies-input").value =
          project.technologies.join(", ");
      }

      clone.querySelector(".project-featured-input").checked =
      ...
      clone.querySelector(".project-details-input").value =
        project.details || "";
      clone.querySelector(".project-challenges-input").value =
        project.challenges || "";
      clone.querySelector(".project-outcomes-input").value =
        project.outcomes || "";
  ...
  function populateSkillsEditor(skillCategories) {
    ...
    skillCategories.forEach((category) => {
      ...
      // Set category name
      clone.querySelector(".skill-category-name-input").value =
        category.category || "";
      ...
      if (category.items && Array.isArray(category.items)) {
        category.items.forEach((skill) => {
          ...
          // Populate skill data
          skillClone.querySelector(".skill-name-input").value =
            skill.name || "";
          skillClone.querySelector(".skill-level-input").value =
            skill.level || 0;
          skillClone.querySelector(".skill-level-text-input").value =
            skill.levelText || "";

          // Handle icon vs iconText
          if (skill.icon === "icon-text" && skill.iconText) {
            skillClone.querySelector(".skill-icon-input").value =
              skill.iconText;
          } else {
            skillClone.querySelector(".skill-icon-input").value =
              skill.icon || "";
          ...
          skillClone.querySelector(".skill-description-input").value =
            skill.description || "";
          skillClone.querySelector(".skill-years-input").value =
      ...
      if (addSkillBtn) {
        addSkillBtn.addEventListener("click", function () {
          const skillsList = this.closest(".skill-category-item").querySelector(
            ".skills-list"
          );
          const skillTemplate = document.querySelector("#skill-item-template");
          const skillClone = document.importNode(skillTemplate.content, true);
  ...
  function populateEducationData(educationItems) {
    ...
    educationItems.forEach((edu) => {
      ...
      clone.querySelector(".education-institution-input").value =
        edu.institution || "";
      clone.querySelector(".education-location-input").value =
        edu.location || "";
      clone.querySelector(".education-start-input").value = edu.startDate || "";
      clone.querySelector(".education-end-input").value =
        (edu.endDate === "Present" ? "" : edu.endDate) || "";
      clone.querySelector(".education-description-input").value =
        edu.description || "";

      // Set achievements as multi-line string
      if (edu.achievements && Array.isArray(edu.achievements)) {
        clone.querySelector(".education-achievements-input").value =
  ...
  function populateExperienceData(experienceItems) {
    ...
    experienceItems.forEach((exp) => {
      ...
      // Populate form fields
      clone.querySelector(".experience-position-input").value =
        exp.position || "";
      clone.querySelector(".experience-company-input").value =
        exp.company || "";
      clone.querySelector(".experience-location-input").value =
        exp.location || "";
      clone.querySelector(".experience-start-input").value =
        exp.startDate || "";
      clone.querySelector(".experience-end-input").value =
        (exp.endDate === "Present" ? "" : exp.endDate) || "";
      clone.querySelector(".experience-description-input").value =
        exp.description || "";

      // Set responsibilities as multi-line string
      if (exp.responsibilities && Array.isArray(exp.responsibilities)) {
        clone.querySelector(".experience-responsibilities-input").value =
  ...
  function populateCertificatesData(certificateItems) {
    ...
    certificateItems.forEach((cert) => {
      ...
      clone.querySelector(".certificate-issuer-input").value =
        cert.issuer || "";
      clone.querySelector(".certificate-date-input").value = cert.date || "";
      clone.querySelector(".certificate-credential-input").value =
        cert.credentialId || "";
      clone.querySelector(".certificate-url-input").value =
        cert.verificationUrl || "";

      // Set skills as comma-separated string
      if (cert.skills && Array.isArray(cert.skills)) {
        clone.querySelector(".certificate-skills-input").value =
          cert.skills.join(", ");
      }
  ...
  function populateGalleryEditor(galleryItems) {
    ...
    galleryItems.forEach((item) => {
      ...
      clone.querySelector(".gallery-description-input").value =
        item.description || "";
      clone.querySelector(".gallery-image-input").value = item.imageUrl || "";
      clone.querySelector(".gallery-thumb-input").value = item.thumbUrl || "";
      clone.querySelector(".gallery-category-input").value =
        item.category || "";
      clone.querySelector(".gallery-featured-input").checked =
        item.featured || false;
  ...
  function populateTestimonialsEditor(testimonialItems) {
    ...
    testimonialItems.forEach((item) => {
      ...
      clone.querySelector(".testimonial-position-input").value =
        item.position || "";
      clone.querySelector(".testimonial-company-input").value =
        item.company || "";
      clone.querySelector(".testimonial-avatar-input").value =
        item.avatar || "";
      clone.querySelector(".testimonial-content-input").value =
        item.testimonial || "";
      clone.querySelector(".testimonial-rating-input").value = item.rating || 5;
      clone.querySelector(".testimonial-project-input").value =
        item.project || "";
      clone.querySelector(".testimonial-featured-input").checked =
        item.featured || false;
